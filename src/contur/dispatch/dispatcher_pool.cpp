...
namespace contur {
    ...
    struct DispatcherPool::Impl
    {
        ...
        [[nodiscard]] bool ensureWorkersStarted()
        {
            ...
            try
            {
                ...
                // Per-worker trace buffers: each worker writes to its own slot, no mutex needed.
                perWorkerTraces.resize(config.hostThreadCount);
        ...
        void resetJobState(JobKind nextJob, const DispatcherLanes &nextLanes, std::size_t nextTickBudget)
        {
        ...
        // Merge per-worker trace buffers and produce a canonical sorted result.
        // Called from waitForJobCompletion() which holds the mutex.
        void mergeWorkerTraces()
        {
            lastTraceEvents.clear();
            for (const auto &wt : perWorkerTraces)
            {
                lastTraceEvents.insert(lastTraceEvents.end(), wt.begin(), wt.end());
            }

            // Sort for canonical ordering:
            //   deterministic mode  -> sequence = laneIndex -> stable 0,1,2,... order
            //   non-deterministic   -> sequence = global monotonic counter -> execution order
            std::sort(lastTraceEvents.begin(), lastTraceEvents.end(), [](const TraceEvent &a, const TraceEvent &b) {
                return a.sequence < b.sequence;
            });

            // Derive deterministic lane order from sorted events.
            lastDeterministicOrder.clear();
            if (config.deterministicMode)
            {
                for (const auto &event : lastTraceEvents)
                {
                    lastDeterministicOrder.push_back(static_cast<std::size_t>(event.sequence));
                }
            }
        }
        ...
        )
        {
            ...
            {
                // Deterministic: static stride assignment guarantees each lane is always
                // processed by the same worker index. Combined with sequence=laneIndex in
                // appendTraceEvent, this makes trace output reproducible across runs.
            ...
            // Non-deterministic: dynamic work stealing via atomic counter.
            while (true)
            {
        ...
        // Appends a trace event to the calling worker's private buffer.
        // No mutex required since each worker only writes to its own slot.
        void appendTraceEvent(std::size_t workerIndex, std::size_t laneIndex, std::string_view operation)
        {
            ...
            // In deterministic mode use laneIndex as sequence so that after the merge
            // sort the canonical order is always 0, 1, 2, ... regardless of thread
            // interleaving.  In non-deterministic mode use a global monotonic counter
            // so that the sequence reflects actual execution order.
            event.sequence = config.deterministicMode ? static_cast<std::uint64_t>(laneIndex)
                                                      : traceSequence.fetch_add(1, std::memory_order_relaxed);
            event.epoch = lastEpoch;
            perWorkerTraces[workerIndex].push_back(std::move(event));
        }
        ...
        std::vector<std::vector<TraceEvent>> perWorkerTraces;
    };
    ...
    Result<void> DispatcherPool::dispatch(const DispatcherLanes &lanes, std::size_t tickBudget)
    {
...

        {
            ...
            impl_->resetJobState(Impl::JobKind::Dispatch, lanes, tickBudget);
        }
    ...
    void DispatcherPool::tick(const DispatcherLanes &lanes)
    {
        ...
        {
            std::lock_guard<std::mutex> lock(impl_->mutex);
            impl_->resetJobState(Impl::JobKind::Tick, lanes, 0);
        }
        impl_->jobReady.notify_all();
        impl_->waitForJobCompletion();
    }
